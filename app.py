...
<style>
    ...
    div[data-testid="stButton"] > button { font-size: 12.5px!important; padding: 0 10px!important; border-radius: 12px!important; background: #1e1e26!important; border: 1px solid #2a2a35!important; color: #ccc!important; height: 42px!important; min-height: 42px!important; }
    div[data-testid="stButton"] > button:hover { background: #2a2a38!important; border-color: #ff6a00!important; color: white!important; }
    ...
    .thinking-avatar { width: 32px; height: 32px; border-radius: 50%; background: linear-gradient(135deg, #ff6a00, #ee0979); display: flex; align-items: center; justify-content: center; }
    .dots { display: flex; gap: 4px; } .dot { width: 6px; height: 6px; border-radius: 50%; background: #ff6a00; animation: bounceDot 1.4s infinite; }
    .ready-dot { display: flex; align-items: center; gap: 6px; font-size: 11px; color: #00ff88; margin: 6px 0 4px 2px; }
    .ready-dot span { width: 6px; height: 6px;
...
def new_chat():
    if st.session_state.messages:
        title = st.session_state.messages[0]["content"][:30] + "..." if isinstance(st.session_state.messages[0]["content"], str) else "Image Chat..."
...
with st.sidebar:
    ...
    for cid, chat in reversed(list(st.session_state.all_chats.items())):
        ...
        if st.button(f"📝 {chat['title']}", key=cid, use_container_width=True):
            ...
            st.rerun()
    st.markdown("---")
    st.markdown('<div class="sidebar-card" style="font-size:12px; color:#888; text-align:center">Made with ❤️ in Belpahar<br>v4.0 • Image + History</div>', unsafe_allow_html=True)
    ...
    with st.chat_message("assistant"):
        ...
        with rc1:
            if st.button("🎓 Student", use_container_width=True, key="role_student"):
                st.session_state.pending_prompt = "I am a Student from Belpahar. Explain things simply, help me study, give notes, and be my study buddy."
                st.rerun()
        with rc2:
            if st.button("💻 Developer", use_container_width=True, key="role_dev"):
                st.session_state.pending_prompt = "I am a Developer from Belpahar. You are my coding partner. Help me with Python, debugging, Streamlit, and clean code."
                st.rerun()
        with rc3:
            if st.button("💭 Dreamer", use_container_width=True, key="role_dream"):
                st.session_state.pending_prompt = "I am a Dreamer from Belpahar with big ideas. Motivate me, help me build my startup Aditya AI, and think big with me."
                st.rerun()
